...
USER_AGENT = {"User-Agent": "ThePressBot/1.0 (+https://thepress.live)"}
...
def is_bad_text(text: str) -> bool:
    lowered = text.lower()
    return any(re.search(pat, lowered) for pat in BAD_IMAGE_PATTERNS)
...
- Do not show raw naked URLs in the body.
...
- thumbnail_search_hint should describe a real event or topical image likely available on Wikimedia Commons.
...
def wiki_thumbnail(query: str) -> tuple[str, str]:
    ...
    r = requests.get(search_url, params=params, timeout=20, headers=USER_AGENT)
    r.raise_for_status()
    data = r.json()
    pages = list((data.get("query", {}) or {}).get("pages", {}).values())
    # Prefer stable ordering for reproducibility.
    ...
    for page in pages:
        ...
        desc_url = page.get("fullurl") or info.get("descriptionurl") or ""
        text_blob = " ".join([title, img_url, desc_url])
        if is_bad_text(text_blob):
            continue
        # Reject SVGs and known generic/file icons.
        parsed = urlparse(img_url).path.lower()
        if parsed.endswith(".svg") or "/icon" in parsed:
            continue
        return img_url, desc_url
...
def download_image(url: str, slug: str, edition_date: str) -> str:
    ...
    if is_bad_text(url):
        raise RuntimeError("Rejected placeholder/generic image URL")
...
<section id=\"daily-intelligent-ai-feed\" class=\"daily-home-section\">
  <div class=\"section-heading-row\">
    <h2 class=\"section-heading\">Today’s 10 new stories</h2>
...
def patch_breaking(stories: list[Story], edition_date: str) -> None:
    ...
    if not path.exists():
        path.write_text("<!doctype html><html><head><meta charset='utf-8'><title>Breaking News — The Press</title><link rel='stylesheet' href='styles.css'></head><body><main></main></body></html>", encoding="utf-8")
    soup = BeautifulSoup(path.read_text(encoding="utf-8"), "html.parser")
    items = []
    for story in stories[:5]:
        items.append(f"<li><p class='eyebrow eyebrow--compact'>{story.section_name}</p><a href='daily/{story.slug}.html'>{story.title}</a><p>{story.dek}</p></li>")
    html_fragment = f"""
...
def build_story(client: OpenAI, section_slug: str, section_name: str, published_label: str, edition_date: str) -> Story | None:
    for _ in range(3):
        payload = call_model(client, section_name, published_label)
        title = payload["title"].strip()
        slug = f"{edition_date}-{slugify(title)}"
        queries = []
        if payload.get("thumbnail_search_hint"):
            queries.append(str(payload["thumbnail_search_hint"]))
        queries.append(title)
        queries.extend([f"{title} {k}" for k in payload.get("keywords", [])[:3]])
        thumb_url = None
        for q in queries:
            try:
                found_url, _ = wiki_thumbnail(q)
                thumb_url = found_url
                break
            except Exception:
                continue
...
def main() -> int:
    ...
    attempts = 0
    i = 0
    max_attempts = story_count * 5
    while len(stories) < story_count and attempts < max_attempts:
        section_slug, section_name = SECTIONS[i % len(SECTIONS)]
        attempts += 1
        i += 1
        story = build_story(client, section_slug, section_name, published_label, edition_date)
        if not story:
            continue
        if story.slug in {s.slug for s in stories}:
            continue
        stories.append(story)
        (DAILY_DIR / f"{story.slug}.html").write_text(build_story_page(story), encoding="utf-8")

    if not stories:
        raise SystemExit("Could not generate any stories with valid legal topical images")
