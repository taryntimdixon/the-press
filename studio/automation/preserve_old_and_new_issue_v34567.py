...
def generation_prompt(section_name: str, date_label: str) -> str:
...
def verification_prompt(payload: dict[str, Any], section_name: str, date_label: str) -> str:
    return f"""
...
def repair_prompt(payload: dict[str, Any], section_name: str, date_label: str, issues: list[str], metrics: dict[str, Any]) -> str:
    return f"""
...
Repair the article by doing more live research, expanding the analysis, improving fact-checking, and fixing the source list. Do not merely add a pile of links; every source must support the article. Keep the writing original and polished.
...
def call_model(client: OpenAI, section_name: str, date_label: str) -> dict[str, Any]:
    payload = request_payload(client, generation_prompt(section_name, date_label))

    for _ in range(FACT_CHECK_PASSES):
        payload = request_payload(client, verification_prompt(payload, section_name, date_label))

    for _ in range(QUALITY_REPAIR_ATTEMPTS):
        issues, metrics = payload_quality_issues(payload)
        if not issues:
            return payload
        print(f"Quality repair needed for {section_name}: {issues}")
        payload = request_payload(client, repair_prompt(payload, section_name, date_label, issues, metrics))
