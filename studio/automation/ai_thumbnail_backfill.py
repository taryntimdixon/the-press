...
2. Leaves real/legal existing images alone.
3. Replaces missing, broken, placeholder, fallback, or SVG article thumbnails with
   an AI-generated editorial illustration matched to the article.
4. Updates the article page, linked homepage/archive/section cards, and common JSON indexes.
...
AI_IMAGE_DIR = ROOT / "assets" / "ai-thumbnails"
...
def path_from_img_src(src: str, html_path: Path) -> Path | None:
    if not src:
        return None
    parsed = urlparse(src)
    if parsed.scheme in {"http", "https", "data", "mailto"}:
        return None
    clean = unquote(parsed.path or src).strip()
    if not clean:
        return None
    if clean.startswith("/"):
...
def looks_like_article_page(path: Path, soup: BeautifulSoup, master_filenames: set[str]) -> bool:
    ...
    if path.name in SKIP_HTML_NAMES or path.name.startswith("section-"):
        return False
    if soup.select_one("article.article-shell, main.article-layout"):
        return True
...
def page_dek(soup: BeautifulSoup, container: Any) -> str:
    for selector in (".article-dek", ".dek", ".story-dek", "meta[name='description']"):
...
def update_cards_in_html_file(path: Path, updates: list[ArticleImageUpdate]) -> bool:
    ...
    for update in updates:
        ...
        for anchor in anchors:
            ...
            if current_src == rel_to_file(update.new_abs_path, path):
                continue
            ...
            if should_update:
                img["src"] = rel_to_file(update.new_abs_path, path)
                img["alt"] = update.alt
                changed = True
...
def recursively_update_json_images(obj: Any, updates_by_url: dict[str, ArticleImageUpdate]) -> bool:
    ...
    if isinstance(obj, dict):
        ...
        if update:
            for key in ("image", "thumbnail", "thumbnail_local", "thumbnailLocal"):
                if key in obj:
...
def main() -> int:
    ...
    print(f"Scanning {len(article_paths)} article pages for missing/bad thumbnails")
    ...
    if not updates:
        print("No missing/bad article thumbnails found")
        return 0
    ...
    print(f"AI thumbnail backfill complete: {len(updates)} page(s) updated; {generated_count} new image(s) generated")
    return 0
