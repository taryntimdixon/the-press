...
def load_json(path: Path, default: Any) -> Any:
    ...
    except Exception as exc:  # pragma: no cover - defensive for malformed manual edits
...
def parse_date(value: Any, fallback_url: str = "") -> datetime | None:
    ...
    # Example: April 17, 2026 3:41 PM -04:00
    match = re.search(
    ...
    if match:
        ...
        if month:
            ...
            return datetime(int(match.group("year")), month, int(match.group("day")), hour, minute, tzinfo=tz).astimezone(timezone.utc)
...
def date_label(dt: datetime | None) -> str:
    if not dt:
        return ""
    return dt.astimezone(timezone.utc).strftime("%B %-d, %Y • %-I:%M %p UTC").replace("AM", "a.m.").replace("PM", "p.m.")
...
def normalize_story(item: dict[str, Any], source: str) -> Story | None:
    ...
    section = title_case_section(item.get("section") or item.get("desk") or item.get("category") or section_from_url(url))
    story_type = clean(item.get("type") or item.get("kind") or ("Daily Issue" if url.startswith("daily/") else "Report"))
    keywords = [clean(k) for k in item.get("keywords", []) if clean(k)] if isinstance(item.get("keywords"), list) else []
    published_raw = item.get("publishedIso") or item.get("published_iso") or item.get("publishedAt") or item.get("published") or item.get("publishedLabel") or item.get("date")
    updated_raw = item.get("updatedIso") or item.get("updated_iso") or item.get("updatedAt") or item.get("updated") or item.get("updatedLabel")
    published_dt = parse_date(published_raw, url)
    updated_dt = parse_date(updated_raw, url)
    published_label = clean(item.get("published") or item.get("publishedLabel") or item.get("published_label")) or date_label(published_dt)
    updated_label = clean(item.get("updated") or item.get("updatedLabel") or item.get("updated_label")) or date_label(updated_dt)
    story_slug = clean(item.get("story_id") or item.get("storyId") or item.get("slug") or story_id_from_url(url))
    ...
    return Story(
        ...
        image=clean(item.get("image") or item.get("imageUrl") or item.get("image_url") or item.get("thumbnail") or item.get("photo")),
...
def merge_all(groups: Iterable[Iterable[Story]]) -> list[Story]:
    ...
    for story in [s for group in groups for s in group]:
        ...
        if key in by_url:
            ...
            by_url[key] = story
    return sorted((s for s in by_url.values() if s.status == "published"), key=lambda s: s.sort_ts, reverse=True)
...
def pick(stories: list[Story], count: int, used_clusters: set[str] | None = None, unique_sections: bool = False, require_image: bool = False, mode: str = "placement") -> list[Story]:
    ...
    if len(out) < count and (unique_sections or require_image):
        out.extend(pick(stories, count - len(out), used_clusters, unique_sections=False, require_image=False, mode=mode))
    return out[:count]


def build_placements(stories: list[Story]) -> dict[str, Any]:
    live_cutoff = NOW - timedelta(days=LIVE_WINDOW_DAYS)
    recent = [story for story in stories if not story.sort_ts or datetime.fromtimestamp(story.sort_ts, timezone.utc) >= live_cutoff]
    ...
    hero = pick([s for s in cluster_pool if s.hero_eligible], 4, used, unique_sections=True, require_image=True)
    ...
    most_read = pick(cluster_pool, 5, set(s.cluster_id for s in [*hero, *secondary]), unique_sections=False, mode="placement")
    editor_used = set(s.cluster_id for s in [*hero, *secondary, *most_read])
    editors = pick(cluster_pool, 4, editor_used, unique_sections=True, mode="editors")

    desks: dict[str, dict[str, Any]] = defaultdict(lambda: {"count": 0, "latest_ts": 0, "story_id": "", "section": ""})
    ...
    desk_pulse = sorted(desks.values(), key=lambda d: (d["latest_ts"], d["count"]), reverse=True)[:10]
...
def main() -> None:
    ...
    parser.add_argument("--refresh-search-index", action="store_true", help="Rewrite search-index.json from the canonical merged index.")
    ...
    live = [story for story in stories if story.sort_ts and datetime.fromtimestamp(story.sort_ts, timezone.utc) >= cutoff]
    if len(live) < 12:
    ...
    print(f"done: {len(stories)} canonical stories, {len(live)} live, {len(archive)} archive-only")
